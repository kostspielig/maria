defmodule MariaWeb.RecipeControllerTest do
  ...
  describe "index" do
    test "lists all recipes", %{conn: conn} do
      conn = conn |> log_in_user(user_fixture()) |> get(~p"/recipes")
  ...
  describe "new recipe" do
    test "renders form", %{conn: conn} do
      conn =  conn |> log_in_user(user_fixture()) |> get(~p"/recipes/new")
  ...
  describe "create recipe" do
    ...
    test "redirects to show when data is valid", %{conn: conn} do
      conn =  conn |> log_in_user(user_fixture()) |> post(~p"/recipes", recipe: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == ~p"/recipes/#{id}"

      conn = conn |> log_in_user(user_fixture()) |> get(~p"/recipes/#{id}")
      assert html_response(conn, 200) =~ "Recipe #{id}"
    end

    @tag :skip
    test "renders errors when data is invalid", %{conn: conn} do
      conn = conn |> log_in_user(user_fixture()) |> post(~p"/recipes", recipe: @invalid_attrs)
      assert html_response(conn, 200) =~ "New Recipe"
    end
  end
  ...
  describe "edit recipe" do
    ...
    test "renders form for editing chosen recipe", %{conn: conn, recipe: recipe} do
      conn =  conn |> log_in_user(user_fixture()) |> get(~p"/recipes/#{recipe}/edit")
  ...
  describe "update recipe" do
    ...
    test "redirects when data is valid", %{conn: conn, recipe: recipe} do
      conn = conn |> log_in_user(user_fixture()) |> put(~p"/recipes/#{recipe}", recipe: @update_attrs)
      assert redirected_to(conn) == ~p"/recipes/#{recipe}"

      conn = conn |> log_in_user(user_fixture()) |> get(~p"/recipes/#{recipe}")
      assert html_response(conn, 200) =~ "some updated description"
    end

    @tag :skip
    test "renders errors when data is invalid", %{conn: conn, recipe: recipe} do
      conn = conn |> log_in_user(user_fixture()) |> put(~p"/recipes/#{recipe}", recipe: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Recipe"
    end
  end
  ...
  describe "delete recipe" do
    ...
    test "deletes chosen recipe", %{conn: conn, recipe: recipe} do
      conn = conn |> log_in_user(user_fixture()) |> delete(~p"/recipes/#{recipe}")
